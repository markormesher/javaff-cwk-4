...
public class BestSuccessorSelectorWithRandom implements SuccessorSelector {

	private static BestSuccessorSelectorWithRandom ss = null;

	public static BestSuccessorSelectorWithRandom getInstance() {
		if (ss == null) ss = new BestSuccessorSelectorWithRandom(); // Singleton, as in NullFilter
		return ss;
	}

	private BestSuccessorSelectorWithRandom() {
	}

	public State choose(Set toChooseFrom) {

		if (toChooseFrom.isEmpty()) return null;

		HashSet jointBest = new HashSet(); // set to store the joint-best states
		...
		while (itr.hasNext()) {
			...
			if (curr.getHValue().compareTo(bestHeuristic) < 0) { // if it has a better heuristic value
				...
				jointBest.add(curr); // it is the joint best, with only itself
		...
		return ((State) (skipThrough.next())); // return state from set
